...
\end{itemize}
Observe that this proof only uses compactness and metricity of $[0,T],$ hence can be generalized to a compact metric domain.
\end{proof}
...
\begin{example}
    ...
    \end{itemize}
    Observe that for the 3rd one needs to see that convex functions valued \(0\) in \(0\) are subadditive.
    This can be seen applying convexity definitions on \(a,b,a+h\) and \(a, a+h, b+h\). It is very elegant to do so using the three point inequality formulation of convexity.
\end{example}
...
\begin{lemma}\label{lem:1varSemicont}
...
\end{lemma}
\begin{proof}[Outline of the proof]
    A counterexample is given by a sequence of saw function with \(n\) angles of heigth \(1/n\).
\end{proof}
\subsubsection{Absolute continuity}

\begin{proposition}[mistake in the book proof]
...
\begin{lemma}\label{lem:1HolSemicont}
...
\begin{proposition}[1-variation of $C^1$ paths]
...
\begin{proposition}[mistake in the book]
...
\begin{proposition}\label{theo:1varL1}
...
\begin{corollary}\label{cor:PLapprox}
